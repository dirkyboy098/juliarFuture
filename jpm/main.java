...
public class Main {
    public static boolean saveUrlContent(String url,String filename){
        try {
            ...
            String routePath = Main.class.getResource(File.separator).getPath();
    ...
    public static List<List<String>> generateModuleList(){
        ...
        String content = "";
        try {
            URL path = Main.class.getResource("version.jpm");
            content = new String(Files.readAllBytes(Paths.get(path.toURI())));
        }
        catch(Exception e) {
            e.printStackTrace();
    ...
    public static void main(String[] args){
        ...
        for(int i=0;i<names_list.size();i++) {
            double newversion = getUrlVersion("https://juliar.org/repo/?geturlversion="+names_list.get(i));
            if (Double.parseDouble(versions_list.get(i)) < newversion) {
                ...
                if (useranswer.toLowerCase() == "yes" || useranswer.toLowerCase() == "y") {
                    boolean success = saveUrlContent(getDownloadUrl(names_list.get(i)), names_list.get(i)+".ju");
                    if(success){
        ...
        try {
            ...
            fileWriter.write("*ignore generated by the jpm *"+System.lineSeparator());
            for(int i=0;i<names_list.size();i++){
